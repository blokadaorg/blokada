            ...
            success = { firstRuleset ->
                ...
                filters.drop(1).forEach { nextFilter ->
                    if (ruleset.size < doGetMemoryLimit()) {
                        ...
                        e(
                            "memory limit reached, skipping ruleset", nextFilter,
...
internal class WildcardBlockade(
    ...
    override fun afterRulesetsBuilt(denyRuleset: Ruleset, allowRuleset: Ruleset) {
        ...
        wildcardAllowRuleset =
            Ruleset().apply { addAll(allowRuleset.filter { it.startsWith("*") }) }
        wildcardDenyRuleset =
            Ruleset().apply { addAll(wildcardDenyRuleset.map { it.removePrefix("*") }) }
        wildcardAllowRuleset =
            Ruleset().apply { addAll(wildcardAllowRuleset.map { it.removePrefix("*") }) }

        v(
            "WildcardBlockade configured, deny/allow:", wildcardDenyRuleset.size,
