...
- **For Android bumps the loop's job is usually diagnosis, not detection.** A pure build break (e.g. bumped androidx requiring a newer AGP / compileSdk than the repo pins) is already caught by the repo's CI `android assemble` job, so the bump's PR is red before you start. Value-add here is the *why* and the *coupling*: which toolchain bump (AGP `#1080`, gradle-wrapper `#1079`) or manual change (compileSdk raise — no Dependabot PR covers it) must land first, and in what order. Put that in the escalation question, not just "build failed". Detection-vs-CI overlap is expected; runtime-behavior validation (the part CI can't do) only starts once it compiles — which on Android means a real device, i.e. the stub above.
