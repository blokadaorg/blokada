...
class DashMainMenu(
    ...
    init {
        onClick = { dashRef ->
            if (menu == null && dashRef is View) {
                ...
                m.add(0, 2, 2, R.string.main_blog_text)
                m.add(0, 3, 3, R.string.main_feedback_text)
                m.add(0, 4, 4, R.string.main_bug_text)
                m.add(0, 5, 5, R.string.main_faq_text)
                m.add(0, 6, 6, R.string.update_about)
    ...
    private val openMenu = { itemId: Int ->
        val dash = when (itemId) {
            ...
            2 -> ui.dashes().firstOrNull { it.id == DASH_ID_BLOG }
            3 -> ui.dashes().firstOrNull { it.id == DASH_ID_FEEDBACK }
            4 -> ui.dashes().firstOrNull { it.id == DASH_ID_BUG }
            5 -> ui.dashes().firstOrNull { it.id == DASH_ID_FAQ }
            6 -> ui.dashes().firstOrNull { it.id == DASH_ID_ABOUT }
            else -> null
        }
        if (dash != null) {
            contentActor.back {
