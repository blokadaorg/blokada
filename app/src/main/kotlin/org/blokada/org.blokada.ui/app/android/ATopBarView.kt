...
import org.blokada.ui.framework.android.AResizeAnimation
import org.blokada.ui.framework.android.doAfter
import org.blokada.ui.framework.android.toPx
...
class ATopBarView(
    ...
    var mode = ATopBarView.Mode.WELCOME
        set(value) {
            when {
                field == value -> Unit
                value == ATopBarView.Mode.WELCOME -> toWelcome { field = value; onModeSwitched() }
                value == ATopBarView.Mode.BACK -> toBack { field = value; onModeSwitched() }
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        halfWidthPx = w / 2
        if (mode == ATopBarView.Mode.WELCOME) toWelcome {}
    }

    private fun toWelcome(after: () -> Unit) {
        logo.visibility = View.VISIBLE
        val a = AResizeAnimation(logo, resources.toPx(196), logo.measuredHeight, square = true)
        a.duration = 300
        ...
        if (mode != ATopBarView.Mode.WELCOME) toBackground(color = null)
        after()
    }

    private fun toBar(after: () -> Unit) {
        logo.visibility = View.VISIBLE
        val a = AResizeAnimation(logo, resources.toPx(64), logo.measuredHeight, square = true)
        a.duration = 300
