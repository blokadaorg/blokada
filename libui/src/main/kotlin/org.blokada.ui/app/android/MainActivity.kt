...
//        staticContext.set(WeakReference(null as Activity?))
    ...
    private val enabledStateListener = object : IEnabledStateActorListener {
        ...
        override fun startDeactivating() {
            ui.infoQueue %= ui.infoQueue() + Info(InfoType.DEACTIVATING)
        }

        override fun finishDeactivating() {
            when {
                s.restart() && s.connection().tethering ->
                    ui.infoQueue %= ui.infoQueue() + Info(InfoType.PAUSED_TETHERING)
                s.restart() -> ui.infoQueue %= ui.infoQueue() + Info(InfoType.PAUSED_OFFLINE)
                else -> ui.infoQueue %= ui.infoQueue() +  Info(InfoType.PAUSED)
            }
        }

        override fun startActivating() {
            ui.infoQueue %= ui.infoQueue() + Info(InfoType.ACTIVATING)
        }
