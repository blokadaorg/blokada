    ...
    override fun onCreate(savedInstanceState: android.os.Bundle?) {
        ...
        val nameVB = EnterDnsNameVB(ktx, accepted = { name ->
            if (servers[0] != null && servers[1] != null) {
                val newDnsChoice = DnsChoice(
                    "custom-dns:" + Base64.encodeToString(
                        name.toByteArray(),
                        Base64.NO_WRAP
                    ), servers.filterNotNull()
                )
        ...
        val ip1VB = EnterIpVB(ktx, first = true, accepted = {
            servers[0] =
                if (it.matches(Regex("^(?:(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])(\\.(?!\$)|\$)){4}\$"))) {
...
class EnterIpVB(
    ...
    private val inputRegex =
        Regex("^(?:(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])\\.){3}(?:(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9]))(?::\\d{1,5})?$")
    ...
    override fun attach(view: SlotView) {
        ...
        view.content = Slot.Content(ktx.ctx.resources.getString(R.string.dns_edit_ip_label),
            description = ktx.ctx.resources.getString(
                if (first) {
