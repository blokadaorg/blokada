...
class FiltersSectionVB(
    ...
    private val filtersUpdated = { filters: Collection<Filter> ->
        ...
        val activeItems = items.filter { it.active }.sortedBy { it.priority }
            .map { FilterVB(it, ktx, onTap = slotMutex.openOneAtATime) }
        val inactiveItems = items.filter { !it.active }.sortedBy { it.priority }
            .map { FilterVB(it, ktx, onTap = slotMutex.openOneAtATime) }
