...
class LeasesDashboardSectionVB(
    ...
    private fun populate(retry: Int = 0) {
        ...
        api.getLeases(currentAccount.id)
            .enqueue(object : retrofit2.Callback<BlockaRestModel.Leases> {
                ...
                override fun onResponse(
                    call: Call<BlockaRestModel.Leases>?,
                    response: Response<BlockaRestModel.Leases>?
                ) {
                    response?.run {
                        when (code()) {
                            200 -> {
