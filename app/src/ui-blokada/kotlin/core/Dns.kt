...
data class JsonDns(
        ...
        val servers: Array<JsonDnsServer>,
        val credit: String?,
        val comment: String?
) {
    override fun equals(other: Any?): Boolean {
...
class JsonDnsSerialiser {
    fun deserialise(repo: String): List<DnsChoice> {
        ...
        val gson = Gson()
        return gson.fromJson(temp_repo, Array<JsonDns>::class.java).map { elm -> elm.index to DnsChoice(
                elm.id,
                elm.servers.map { server ->
                    ipStringToAddress(server.ip)
                }.toList(),
                elm.active,
                elm.usesIpv6,
                elm.credit,
