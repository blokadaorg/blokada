...
- **iOS-first.** `device-log` and the real-device `appium` path are iOS only. Android validation (`make -C android apk-six-debug`, `adb install`, `adb logcat` inspection) is a documented follow-up, not built yet — for an Android bump, run Stages A–B, then escalate the on-device check.
