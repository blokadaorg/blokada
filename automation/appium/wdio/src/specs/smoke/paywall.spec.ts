...
import { compareToGolden, waitForStableScreen } from "../../support/golden.js";
...
describe("Smoke: paywall on inactive account", () => {
  ...
  it("presents the native paywall when enabling protection", async () => {
    ...
    // Since SDK 4 the flow runtime finishes presenting later than a fixed pause
    // covers — a mid-transition frame fails the golden with everything shifted
    // by the status-bar height — so wait for the frame to stop changing.
    await driver.pause(1500);
    await waitForStableScreen();
