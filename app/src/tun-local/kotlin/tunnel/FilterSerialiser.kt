...
class JsonFilterSerialiser {
    fun deserialise(repo: String): Set<Filter> {
        ...
        try {
            val jsonObj = JSONObject(repo)

            val jsonFilters = jsonObj.getJSONArray("filters")
            for (i in 0 until jsonFilters.length()) {
                ...
                false,
                        i,
                    credit = jsonSource.getString("credit"),
                    customName = jsonSource.getString("name"),
                    customComment = jsonSource.getString("comment")
                ))
            }
