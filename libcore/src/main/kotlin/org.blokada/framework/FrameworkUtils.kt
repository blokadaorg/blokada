...
internal fun hasCompleted(f: () -> Unit): Boolean {
    return try { f(); true } catch (e: Exception) { false }
}
...
internal fun newSingleThreadedKContext(j: IJournal, prefix: String): KContext {
...
fun newConcurrentKContext(j: IJournal?, prefix: String, tasks: Int): KContext {
    return Kovenant.createContext {
        callbackContext.dispatcher = buildDispatcher {
