...
sealed class BlokadaNotification(
    val id: Int, val channel: NotificationChannels,
...
class FilteredNotification(reason: String, counter: Int = 0) :
    BlokadaNotification(1, NotificationChannels.FILTERED,
        create = { ctx ->
            val b = NotificationCompat.Builder(ctx)
            b.setContentTitle(
                ctx.resources.getString(
                    R.string.notification_keepalive_title,
                    counter
                )
            )
            ...
            val actionWhitelist = NotificationCompat.Action(
            ...
            val actionNotificationsOff = NotificationCompat.Action(
...
class UsefulKeepAliveNotification(val count: Int, val last: String) : BlokadaNotification(2,
    ...
    create = { ctx ->
        ...
        if (Product.current(ctx) == Product.GOOGLE) {
            ...
            val intent = Intent(ctx, ANotificationsToggleService::class.java).putExtra(
                "new_state",
                !t.enabled()
            )
            ...
            if (t.enabled()) {
                b.addAction(
                    R.drawable.ic_stat_blokada,
                    ctx.resources.getString(R.string.notification_keepalive_deactivate),
                    statePendingIntent
                )
            } else {
                b.addAction(
                    R.drawable.ic_stat_blokada,
                    ctx.resources.getString(R.string.notification_keepalive_activate),
                    statePendingIntent
                )
            }
    ...
    create = { ctx ->
        ...
        b.setStyle(
            NotificationCompat.BigTextStyle()
                .bigText(ctx.getString(R.string.notification_expired_description))
        )
        b.setSmallIcon(R.drawable.ic_stat_blokada)
...
class LeaseExpiredNotification : BlokadaNotification(5, NotificationChannels.BLOCKA_VPN,
    create = { ctx ->
        ...
        b.setStyle(
            NotificationCompat.BigTextStyle()
                .bigText(ctx.getString(R.string.notification_lease_expired_description))
        )
        b.setSmallIcon(R.drawable.ic_stat_blokada)
        b.priority = NotificationCompat.PRIORITY_MAX
        b.setVibrate(LongArray(0))
