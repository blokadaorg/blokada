...
begin
  ...
  warn("       This script needs the same fastlane install `make promote-android` uses.")
  exit(1)
end
...
begin
  ...
  begin
    ...
    # Read-only check: never leave the throwaway edit hanging around. A failure
    # to clean it up must not mask the result of the check itself.
    begin
