...
import 'package:common/src/shared/ui/action_sheet.dart';
      ...
      return Stack(
        children: [
          ...
          SizedBox(
            ...
            height: buttonSize,
            child: Semantics(
              identifier: AutomationIds.powerToggle,
              button: true,
              ...
              child: Touch(
                onTap: () {
                  if (!status.isWorking()) {
                    setState(() {
                      log(Markers.userTap).trace("tappedPowerButton",
                          (m) async {
                        if (!status.isActive() || _account.isFreemium) {
                          ...
                          showPauseActionSheet(context, onSelected: (duration) {
                            log(Markers.userTap)
                                .trace("tappedPowerButtonDialog", (m) async {
...
class PowerButtonPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (isFreemium) {
      // ring freemium
      canvas.drawCircle(Offset(size.width / 2, size.height / 2),
          size.width / 2 - ringWidth, freemiumRingPaint);
    }
