...
class AState(
    ...
    override val connection = newProperty(kctx, { Connection(
            connected = isConnected(ctx),
    ...
    private val filtersRefresh = { it: List<Filter> ->
        ...
        val builtinFilters = serialiser.deserialise(
                load({ openUrl(c.repoURL, c.fetchTimeoutMillis) })
        )
    ...
    override val tunnelPermission = newProperty(kctx, {
        hasCompleted { checkTunnelPermissions(ctx) }
    })
    ...
    override val localised = newPersistedProperty(kctx, ALocalisedPersistence(ctx,
            ...
            zeroValue = { Localised(
                    content = URL("http://localhost/content"),
